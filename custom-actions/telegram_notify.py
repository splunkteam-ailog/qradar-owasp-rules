...
Параметры Custom Action (добавить в UI):
  ...
  - chat_id       : string  : ID чата (981968417)
...
# БАГ 1 БЫЛ: токен был захардкожен прямо в URL — небезопасно.
# ИСПРАВЛЕНО: URL строится динамически из параметра bot_token.
...
def get_owasp_tag(rule_name: str) -> str:
    for key, value in OWASP_CATEGORIES.items():
        if key in rule_name:
            return f"#{key.replace(' ', '_')} — {value}"
    return "#OWASP_Unknown"
...
def build_message(params: dict) -> str:
    ...
    # БАГ 2 БЫЛ: magnitude передавался как строка в int() без strip(),
    # если QRadar добавлял пробел — падал с ValueError.
    # ИСПРАВЛЕНО: strip() перед конвертацией + fallback.
    try:
    ...
    owasp_tag   = get_owasp_tag(rule_name)
    ...
    # БАГ 3 БЫЛ: в Markdown-тексте символы _ и [ без экранирования
    # вызывали ошибку парсинга Telegram и сообщение не отправлялось.
    # ИСПРАВЛЕНО: используем HTML parse_mode вместо Markdown —
    # он надёжнее и не требует экранирования спецсимволов.
    message = (
        f"{sev_emoji} <b>QRADAR OFFENSE ALERT</b> {sev_emoji}\n"
        f"{'─' * 30}\n\n"
        f"🆔 <b>Offense ID:</b> <code>{offense_id}</code>\n"
        f"📋 <b>Rule:</b> {rule_name}\n"
        f"🏷 <b>Category:</b> {owasp_tag}\n\n"
        f"📝 <b>Description:</b>\n"
